...
func (p *ParallelStateProcessor) Process(block *types.Block, statedb *state.StateDB, cfg vm.Config) (types.Receipts, []*types.Log, uint64, error) {
	...
	coinbase, _ := p.chain.Engine().Author(header)
	...
	profile := false
	result, err := blockstm.ExecuteParallel(tasks, profile, metadata, p.chain.parallelSpeculativeProcesses, nil)
	...
	for _, task := range tasks {
		...
		if task.shouldRerunWithoutFeeDelay {
			...
			*statedb = *backupStateDB
			...
			_, err = blockstm.ExecuteParallel(tasks, false, metadata, p.chain.parallelSpeculativeProcesses, nil)
